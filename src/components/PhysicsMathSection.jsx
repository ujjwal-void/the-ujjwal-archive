...
import { Atom, Copy, Check, Cpu } from 'lucide-react';
    ...
    <div>
      <div style={{ marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.4rem' }}>
          <span className="meta-tag">PHYSICS & MATHEMATICS</span>
        ...
        <h2 style={{ fontSize: '2.2rem', marginBottom: '0.4rem' }}>Physics & Mathematics Notes</h2>
          ...
          <div key={item.id} className="card" style={{ padding: '2rem' }}>
            ...
            <div style={{ marginBottom: '1.2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
                <h4 style={{ fontSize: '0.82rem', color: 'var(--text-main)', textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>MATHEMATICAL FORMULA / EQUATION</h4>
                <button className="btn-secondary" onClick={() => handleCopyFormula(item.id, item.formula)} style={{ fontSize: '0.78rem', padding: '0.3rem 0.8rem', gap: '0.3rem', fontFamily: 'var(--font-mono)' }}>
                  {copiedId === item.id ? <Check size={14} style={{ color: 'var(--accent-emerald)' }} /> : <Copy size={14} />}
                  {copiedId === item.id ? 'Copied' : 'Copy Formula'}
                </button>
              </div>
              <div className="code-block-container">
                <pre><code>{item.formula}</code></pre>
              </div>
            </div>

            {/* CS Connection Card */}
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.8rem', background: 'var(--bg-secondary)', padding: '1rem 1.2rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-muted)', marginBottom: '1rem' }}>
              <Cpu size={20} style={{ color: 'var(--text-main)', flexShrink: 0, marginTop: '0.2rem' }} />
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-main)', textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>COMPUTER SCIENCE & AI CONNECTION:</span>
                <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginTop: '0.2rem', lineHeight: '1.5', fontWeight: 500 }}>{item.csConnection}</p>
              </div>
            </div>
            ...
            {item.videoUrl && (
              <div style={{ marginBottom: '1.4rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.5rem' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-main)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase' }}>
