...
export default function LinktreeSection({ onNavigate }) {
  ...
  const getIcon = (iconName, size = 24) => {
    switch (iconName) {
      case 'Sparkles': return <Sparkles size={size} style={{ color: 'var(--accent-indigo)' }} />;
      case 'FileText': return <FileText size={size} style={{ color: 'var(--text-main)' }} />;
      case 'Atom': return <Atom size={size} style={{ color: 'var(--accent-indigo)' }} />;
      case 'Code2': return <Code2 size={size} style={{ color: 'var(--text-main)' }} />;
      case 'Github': return <GithubIcon size={size} style={{ color: 'var(--text-main)' }} />;
      case 'Linkedin': return <LinkedinIcon size={size} style={{ color: 'var(--text-main)' }} />;
      case 'Briefcase': return <Briefcase size={size} style={{ color: 'var(--text-main)' }} />;
      case 'Cpu': return <Cpu size={size} style={{ color: 'var(--text-main)' }} />;
      case 'BookOpen': return <BookOpen size={size} style={{ color: 'var(--text-main)' }} />;
      default: return <LinkIcon size={size} style={{ color: 'var(--text-main)' }} />;
    }
  };
  ...
  return (
    <div className="linktree-standalone">
      ...
      <div className="linktree-actions-bar">
        <button className="btn-secondary" onClick={handleCopy} style={{ fontSize: '0.85rem', padding: '0.55rem 1.2rem', gap: '0.4rem' }}>
          {copied ? <Check size={16} style={{ color: 'var(--accent-emerald)' }} /> : <Copy size={16} />}
          {copied ? 'Copied Linktree Link!' : 'Copy Linktree Link'}
        </button>
        <button className="btn-primary" onClick={() => onNavigate('home')} style={{ fontSize: '0.85rem', padding: '0.55rem 1.2rem', gap: '0.4rem' }}>
        ...
        {LINKTREE_LINKS.map(link => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '100%' }}>
              <div className="linktree-icon-badge">
                {getIcon(link.icon, 24)}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: '1.02rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  {link.title}
                  {link.url.startsWith('http') && <ExternalLink size={14} style={{ color: 'var(--text-dim)', flexShrink: 0 }} />}
                </div>
                <div style={{ fontSize: '0.84rem', color: 'var(--text-muted)', lineHeight: '1.3' }}>{link.description}</div>
              </div>
            </div>
            {link.badge && (
              <span className="meta-tag" style={{ fontSize: '0.72rem', flexShrink: 0, marginLeft: '0.5rem' }}>{link.badge}</span>
            )}
          </div>
        ))}
      </div>
